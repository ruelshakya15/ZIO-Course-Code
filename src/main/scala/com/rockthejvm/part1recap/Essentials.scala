...
object Essentials {
  ...
  //instructions(side effects) vs expression
  ...
  val anAttempt = Try(/* something that might throw */ 42) // Success(42)/ Failure(..)
  ...
  val optionDescription: String = anOption match
    case Some(value) => "the option is not empty: $value"
    case None => "the option is empty"

  //  Futures
  //  import scala.concurrent.ExecutionContext.Implicits.global // since scala 2.13 don't use this implicit instead define own as below
  given ec: ExecutionContext = ExecutionContext.fromExecutorService(Executors.newFixedThreadPool(8)) //Executors is a Java method
  val aFuture = Future{
    // a bit of code
    42
  }
  ...
  aFuture.onComplete { // receive a Try[] object
