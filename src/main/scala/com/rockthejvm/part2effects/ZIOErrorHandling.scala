package com.rockthejvm.part2effects

import zio.*

import java.io.IOException
import java.net.NoRouteToHostException
import scala.util.{Failure, Success, Try}

object ZIOErrorHandling extends ZIOAppDefault {

  // ZIOs can fail
  val aFailedZIO = ZIO.fail("Something went wrong")
  val failedWithThrowable = ZIO.fail(new RuntimeException("Boom!"))
  val failedWithDescription = failedWithThrowable.mapError(_.getMessage) // So we can get a string instead of the program exception

  // attempt: run an effect that might throw an exception
  val badZIO = ZIO.succeed{
    println("Trying something")
    val string: String = null
    string.length
  } // this is bad

  // USE ATTEMPT if you're ever unsure whether your code might throw
  val anAttempt: ZIO[Any, Throwable, Int] = ZIO.attempt{
    println("Trying something")
    val string: String = null
    string.length
  }

  // effectfully catch errors
  val catchError = anAttempt.catchAll(e => ZIO.succeed(s" Returning a different value because $e"))
  val catchSelectiveErrors = anAttempt.catchSome{
    case e: RuntimeException => ZIO.succeed(s"Ignoring runtime exceptions: $e")   // some explanation of returned type being LCA of anAttempt and the code-block of catchSome explained in VIDEO [TO understand]
    case _ => ZIO.succeed("Ignoring everything else")
  }

  // chain effects
  val aBetterAttempt: ZIO[Any, Nothing, Int] = anAttempt.orElse(ZIO.succeed(56)) // if ZIO fails return another ZIO (notice Nothing in Error Channel as exception mitigated)
  // fold: handle both success and failure
  val handleBoth: ZIO[Any, Nothing, String] = anAttempt.fold(ex => s"Something bad happened:$ex", value => s"Length of the string was ${value}")
  // effectful fold: foldZIO (instead of plain value return another ZIOEffect )
  val handleBoth_v2 = anAttempt.foldZIO(
    ex => ZIO.succeed(s"Something bad happened:$ex"),
    value => ZIO.succeed(s"Length of the string was ${value}")
  )

  /*
    Conversion between Option/Try/Either to ZIO
   */

  // try
  val aTryToZIO: ZIO[Any, Throwable, Int] = ZIO.fromTry(Try(42/0)) // can fail with Throwable

  // either -> ZIO
  val anEither: Either[Int, String] = Right("Success!")
    //left() in Error & right() in value channel
  val anEitherToZIO: ZIO[Any, Int, String] = ZIO.fromEither(anEither)
  //ZIO with throwable in Error channel as  -> ZIO with Either as the value channel
  val eitherZIO: ZIO[Any, Nothing, Either[Throwable, Int]] = anAttempt.either
  // reverse of above[left of Either turned -> error channel]
  val anAttempt_v2: ZIO[Any, Throwable, Int] = eitherZIO.absolve

  // option -> ZIO        None vanna khojya here in error channel
  val anOption: ZIO[Any, Option[Nothing], Int] = ZIO.fromOption(Some(42))


  /**
   * Exercise: implement a version of fromTry, fromOption, fromEither, either, absolve
   * using fold and foldZIO
   */

  def try2ZIO[A](atry: Try[A]): Task[A] = atry match
      case Success(value) => ZIO.succeed(value)
      case Failure(exception) => ZIO.fail(exception)

  def either2ZIO[A, B](aEither: Either[A, B]): ZIO[Any, A, B] = aEither match
    case Left(l) => ZIO.fail(l)
    case Right(r) => ZIO.succeed(r)

  def option2ZIO[A](anOption: Option[A]): ZIO[Any, Option[Nothing], A] = anOption match
    case Some(value) => ZIO.succeed(value)
    case None => ZIO.fail(None)

  def zio2zioEither[R,A,B](zio: ZIO[R, A, B]): ZIO[R, Nothing, Either[A, B]] = zio.foldZIO(
    error => ZIO.succeed(Left(error)),
    value => ZIO.succeed(Right(value))
  )

  def absolveZIO[R,A,B](eitherRep: ZIO[R, Nothing, Either[A,B]]): ZIO[R, A, B] = eitherRep.flatMap {
      case Left(value) => ZIO.fail(value)
      case Right(value) => ZIO.succeed(value)
    }

  //**************************************************************2ND VIDEO************************************************************************
  /*
    Errors = failure present in the ZIO type signature ("checked exception")- EG (ZIO.fail(...))
    Defects = failures that are unrecoverable, unforeseen, NOT present in the ZIO type signature (EG "badZIO" above line no 15)

      ZIO[R,E,A] can finish with Exit[E,A]
      - Success[A] containing a value
      - Cause[E]
        - Fail[E] containing the error
        - Die(t: Throwable)  which was unforeseen
   */
  val divisionByZero: UIO[Int] = ZIO.succeed(1 / 0) // defect

  val failedInt: ZIO[Any, String, Int] = ZIO.fail("I failed!") // error
  val failureCauseExposed: ZIO[Any, Cause[String], Int] = failedInt.sandbox // expose full cause of failure
  val failureCauseHiddent: ZIO[Any, String, Int] = failureCauseExposed.unsandbox
  // fold with cause
  val foldedWithCause = failedInt.foldCause(
    cause => s"this failed with ${cause.defects}", //.defects() extracts List(non recoverable errors)
    value => s"this succeeded with $value"
  )

  val foldedWithCause_v2 = failedInt.foldCauseZIO(
    cause => ZIO.succeed(s"this failed with ${cause.defects}"), //.defects() extracts List(non recoverable errors)
    value => ZIO.succeed(s"this succeeded with $value")
  )

  /*
     Good practice:
     - at a lower level, your "errors" should be treated
     - at a higher level, you should hide "errors" and assume they are unrecoverable
    */
  def callHTTPEndpoint(url: String): ZIO[Any, IOException, String] =
    ZIO.fail(new IOException("no internet, dummy!"))

  // convert error -> defects
  val endpointCallWIthDefects: ZIO[Any,Nothing, String] =
    callHTTPEndpoint("rockthejvm.com").orDie // all error are now defects

  // refining the error channel
  def callHTTPEndpointWideError(url: String): ZIO[Any, Exception, String] =
    ZIO.fail(new IOException("No internet!!"))

  def callHTTPEndpoint_v2(url: String): ZIO[Any, IOException, String] =
    callHTTPEndpointWideError(url).refineOrDie[IOException]{ // partial fxn ( pattern match OR Die)
      case e:IOException => e
      case _: NoRouteToHostException  => new IOException(s"No route to host to $url, can't fetch page")
    }

  // reverse: turn defects into -> error channel
  val endpointCallWithError: ZIO[Any, String, String] = endpointCallWIthDefects.unrefine { // take partial fxn with "throwables" as defects are considered as throwables
    case e: Throwable => e.getMessage // according to code here error channel type is decided
  }

  /*
    Combine effects with different errors
   */
  trait AppError
  case class IndexError(message: String) extends AppError
  case class DbError(message: String) extends AppError
  val callApi: ZIO[Any, IndexError, String] = ZIO.succeed("page: <html></html>")
  val queryDb: ZIO[Any, DbError, Int] = ZIO.succeed(1)
  val combined: ZIO[Any, Product, (String, Int)] = for {
    page <- callApi
    rowsAffectedd <- queryDb
  } yield (page, rowsAffectedd) //  lost type safety [because eChannel => Product(LCA)]

  /*
      Solutions:
        - [Probably the best SOLUTION] design an error model     -> eg. (trait App Error that extends errors)
        - use Scala 3 union types                                -> eg. (ZIO[Any, IndexError | DbError, (String, Int)]
        - .mapError to some common error type                    -> eg. (callApi.mapError(_ => IndexError("hello"))
    */


  /**
   *
   * Exercises
   */
  // 1 - make this effect fail with a TYPED error
  val aBadFailure = ZIO.succeed[Int](throw new RuntimeException("this is bad"))
  val aBetterFailure: ZIO[Any, Cause[Nothing], Int] = aBadFailure.sandbox // exposes the defect in the Cause
  val aBetterFailure_v2: ZIO[Any, Throwable, Int] = aBadFailure.unrefine { // surfaces out the exception in the error channel
    case e: Throwable => e
  }

  //  2 - transform a zio into another zio with a narrower exception type
  def ioException[R,A](zio: ZIO[R, Throwable, A]): ZIO[R, IOException, A] =
    zio.refineOrDie{
      case ioe: IOException => ioe // if IOexception occurs return that in eChannel else -> Defect
    }
    // zio.orElse(ZIO.fail(new IOException("narrower ZIO"))) // OWN i think this is ok too

  // 3
  def left[R, E, A, B](zio: ZIO[R,E,Either[A,B]]): ZIO[R, Either[E,A], B] =
    zio.foldZIO(
      e => ZIO.fail(Left(e)),
      {
        case Left(l) => ZIO.fail(Right(l))
        case Right(r) => ZIO.succeed(r)
      }
    )

  // 4
  val database = Map (
    "daniel" -> 123,
    "alice" -> 789
  )
  case class QueryError(reason: String)
  case class UserProfile(name: String, phone: Int)

  def lookupProfile(userId: String): ZIO[Any, QueryError, Option[UserProfile]] =
    if(userId != userId.toLowerCase())
      ZIO.fail(QueryError("user ID format is invalid"))
    else
      ZIO.succeed(database.get(userId).map(phone => UserProfile(userId, phone)))

  // surface out all the failed cases of this API
  def betterLookupProfile(userId: String): ZIO[Any, Option[QueryError], UserProfile] =
    lookupProfile(userId).foldZIO(
      error => ZIO.fail(Some(error)),
      profileOption => profileOption match
        case Some(profile) => ZIO.succeed(profile)
        case None => ZIO.fail(None)
    )

  // IN-BUILT ZIO to do same as above ^^^ ".some"
  def betterLookupProfile_v2(userId: String): ZIO[Any, Option[QueryError], UserProfile] =
    lookupProfile(userId).some

  override def run: ZIO[Any with ZIOAppArgs with Scope, Any, Any] = endpointCallWIthDefects.debug
}
