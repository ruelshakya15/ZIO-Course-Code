...
object Schedules extends ZIOAppDefault {
  ...
  // exponential backoff   (for cases: like if server down and all ZIOs tries to reestablish connection resulting in increased load and server down again)
  val exBackoffSchedule = Schedule.exponential(1.second, 2.0)  // 1s interval with 2.0 multiplier
  val fibSchedule = Schedule.fibonacci(1.second) // 1s, 1s, 2s, 3s, 5s, ...

  // combinators (using operators)
  ...
  val totalElapsed = Schedule.spaced(1.second) >>> Schedule.elapsed.map(time => println(s"total time elapse: $time"))
